...
export default {
  ...
  methods: {
      runTests: function() {
        ...
        mocha.describe('component functions', () => {

        mocha.before('setup of some components', async function () {
        ...
        mocha.it('test if comps exist in collection', async () => {
            const comps = allComponents.getAll();
            comps.should.have.length(3);
        });

        mocha.it('test if default comp values have been loaded', async () => {
        ...
        mocha.it('Checking if a valid connection component returns true', async () => {
        ...
        mocha.it('Checking if a invalid connection component returns false', async () => {
        ...
        mocha.describe('Save / Load functions', () => {
        // mocha.before('setup of some components', async function () {

        // });
        mocha.it('test saving project to JSON', async () => {
            // await saveLoadController.saveEventToFile();
        });
        mocha.it('test loading project from JSON', async () => {
        ...
        mocha.describe('#1 networking calculation functions', function() {
        this.timeout(20000);
        var hosts;
        var subnets;
        var subnetmask;
        ...
        mocha.it('Test binary to decimal conversion function', async () => {
        ...
        mocha.it('Test decimal to binary conversion function', async () => {
        ...
        mocha.it('Test "checkAvailableIPAddress" function', async () => {
        ...
        mocha.describe('#2 networking calculation functions', function() {
        var hosts;
        var subnets;
        var subnetmask;
        var supernetmask;
        ...
        mocha.it('Test #2 for calculating number of hosts', async () => {
            hosts = networkController.calculateAllHost();
            expect(hosts).to.equal(15);
        });

        mocha.it('Test #2 for calculating number of subnets', async () => {
            subnets = networkController.calculateAllSubnets();
            expect(subnets).to.equal(15);
        });
        mocha.it('Test #2 for calculating subnetmask', async () => {
            subnetmask = networkController.calculateSubnetMask();
            expect(subnetmask).to.equal('255.255.255.240');
        });
        mocha.it('Test #2 for calculating supernetmask', async () => {
            supernetmask = networkController.calculateSupernetMask(subnets);
            expect(supernetmask).to.equal('255.255.255.224');
        });
        mocha.it('Test #2 for calculating max number of IP addresses', async () => {
            var max_number_hosts = networkController.calculateTotalIPAddresses(subnetmask);
            expect(max_number_hosts).to.equal(14);
        });

        mocha.it('Test #2 for calculating max number of subnets', async () => {
            var max_number_subnets = networkController.calculateTotalNumberSubnets(supernetmask);
